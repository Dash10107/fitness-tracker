...
for label in df['label'].unique():
    ...
    fig,ax  = plt.subplots()
    plt.plot(subset_df[:100]["acc_y"].reset_index(drop=True), label=label)
    plt.legend()
    plt.show()
# --------------------------------------------------------------
...
combined_plot_df = combined_df[['acc_x', 'acc_y', 'acc_z']].plot(ax=ax[0]) 
combined_plot_df = combined_df[['gyr_x', 'gyr_y', 'gyr_z']].plot(ax=ax[1]) 
